...
const StudentProfileSetup = () => {
  ...
  const fillInitialData = async() => {
    try {
      ...
      (document.getElementById("crn") as HTMLInputElement).value = response.data.crn;
      (document.getElementById("urn") as HTMLInputElement).value = response.data.urn;
      (document.getElementById("branch") as HTMLInputElement).value = response.data.branch;
      (document.getElementById("batch") as HTMLInputElement).value = response.data.batch;
      (document.getElementById("email") as HTMLInputElement).value = response.data.email;
      (document.getElementById("phone") as HTMLInputElement).value = response.data.phone;
      (document.getElementById("gender") as HTMLInputElement).value = response.data.gender;
  ...
  // Handle Section input validation (1 letter followed by 1 digit)
  const handleSectionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === "" || regex.test(value)) {
      ...
      setSectionError("Invalid format. Must be 1 letter followed by 1 digit.");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const batch = parseInt((document.getElementById("batch") as HTMLSelectElement)?.value);
    ...
    if (!name || !branch || !crn || !urn || !batch || !email || !phone || !gender || !password || !confirmPassword) {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/students/${stuId}`, {
        method: "PATCH",
        headers: {
